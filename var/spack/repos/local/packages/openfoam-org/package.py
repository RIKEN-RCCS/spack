...
class OpenfoamOrg(Package):
    ...
    def test_openfoamorg(self):
        ...
        exe_name = "blockMesh"
        app = which(exe_name)
        out = app(*opts, output=str.split, error=str.split)
        assert "End" in out
        exe_name = "topoSet"
        app = which(exe_name)
        out = app(*opts, output=str.split, error=str.split)
        assert "End" in out
        exe_name = "refineHexMesh"
        opts.extend(["c0","-overwrite"])
        app = which(exe_name)
        out = app(*opts, output=str.split, error=str.split)
        assert "End" in out
        opts = []
        exe_name = "decomposePar"
        opts.extend(["-cellDist"])
        app = which(exe_name)
        out = app(*opts, output=str.split, error=str.split)
        assert "End" in out
        opts = []
        exe_name = "mpirun"
        opts.extend(["-n","5","simpleFoam","-parallel"])
        app = which(exe_name)
        out = app(*opts, output=str.split, error=str.split)
        assert "Finalising parallel run" in out
        opts = []
        exe_name = "reconstructPar"
        app = which(exe_name)
        out = app(*opts, output=str.split, error=str.split)
        assert "End" in out
