#l Copyright 2013-2023 Lawrence Livermore National Security, LLC and other
...
class Lammps(CMakePackage, CudaPackage, ROCmPackage):
    ...
    ### subroutine "license" is not implemented in spack-v0.21
    ...
    supported_packages = {
        ...
        "user-cgdna": {"when": "@20170504:20210929"},
    ...
    conflicts(
        "%fj@:4.3.0a",
        msg="This package file is available when Fujitsu compiler >= 4.3.1")
    conflicts(
        "+fj_loopopt", when="%fj@:4.4.0")
    ### [RIST] added part ###
    ...
    patch("rist-23Jun2022.patch", when="@20220623 %fj +fj_loopopt")   ### this patch does not contain the result for tersoff tuning
    patch("rist-02Aug2023.patch", when="@20230802 %fj +fj_loopopt")
    patch("rist-29Aug2024.patch", when="@20240829 %fj +fj_loopopt")

    # patch("rist-29Oct2020-fjbugfix-20230329.patch", when='@20201029 %fj +user-ptm')
    patch("rist-29Oct2020-fjbugfix-20230329.patch", when='@20201029 %fj@:4.8.1 +user-ptm')
    patch("rist-23Jun2022-fjbugfix.patch", when='@20220623: %fj@:4.8.1 +ptm')
    # patch("rist-02Aug2023-fjbugfix.patch", when='@20230802: %fj@:4.8.1 +ptm')
    # patch("rist-29Aug2024-fjbugfix.patch", when='@20240829 %fj@:4.8.1 +ptm')

    patch("rist-29Oct2020-exceptions.patch", when='@20201029 %fj +exceptions')
    patch("rist-23Jun2022-exceptions.patch", when='@20220623 %fj +exceptions')
    patch("rist-02Aug2023-exceptions.patch", when='@20230802 %fj +exceptions')   ### sha256 error
    patch("rist-29Aug2024-fugaku.patch", when='@20240829 %fj')
    ### [RIST] added part ###
    ...
    def cmake_args(self):
        ...
        ### [RIST] added part ###
        # if "fj@4.3.1:" in spec:
        if spec.satisfies("%fj@4.3.1:"):
            flagstr  = "-Nclang -std=c++14 -stdlib=libstdc++ -fsigned-char "
            flagstr += "-march=armv8.3-a+sve -mcpu=a64fx -mtune=a64fx -ffj-largepage "
            ...
            args.append(self.define("CMAKE_CXX_FLAGS_RELWITHDEBINFO",
                        "-g " + flagstr + ompfstr + optfstr + infostr))
            args.append(self.define("CMAKE_CXX_FLAGS_RELEASE",
                        flagstr + ompfstr + optfstr + infostr))
            args.append(self.define("CMAKE_CXX_FLAGS_DEBUG",
                        "-g " + flagstr + "-O0 " + ompfstr))
            args.append(self.define("CMAKE_CXX_FLAGS_MINSIZEREL",
                        flagstr + "-O2 -ffast-math " + ompfstr))
            # args.append("-DCMAKE_CXX_FLAGS_RELWITHDEBINFO=" + "-g" +
            #             flagstr + ompfstr + optfstr + infostr)
            # args.append("-DCMAKE_CXX_FLAGS_RELEASE=" +
            #             flagstr + ompfstr + optfstr + infostr)
            # args.append("-DCMAKE_CXX_FLAGS_DEBUG=" + "-g" +
            #             flagstr + "-O0 " + ompfstr)
            # args.append("-DCMAKE_CXX_FLAGS_MINSIZEREL=" +
            #             flagstr + "-O2 -ffast-math " + ompfstr)
        ### [RIST] added part ###
