...
class Gsl(AutotoolsPackage, GNUMirrorPackage):
    ...
    def test_gsl(self):
        ...
        os.environ['C_INCLUDE_PATH'] = self.prefix.include
        os.environ['LIBRARY_PATH'] = self.prefix.lib
        os.environ['LD_LIBRARY_PATH'] = self.prefix.lib
        os.chdir(test_dir)
        opts = []
        exe_name = "gcc"
        opts.extend(["ex.c","-lgsl","-lm","-lgslcblas"])
        gsl = which(exe_name)
        out = gsl(*opts, output=str.split, error=str.split)
        opts = []
        exe_name = "./a.out"
        gsl = which(exe_name)
        out = gsl(*opts, output=str.split, error=str.split)
        assert "J0(5) = -1.775967713" in out
