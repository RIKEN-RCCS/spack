...
class Wrf(Package):
    ...
    def flag_handler(self, name, flags):
        ...
        if self.spec.satisfies("@3.9.1.1: %gcc@10:") and name == "fflags":
            if self.spec.satisfies("%gcc@14.1.0") and platform.machine() in ["aarch64", "arm64"]:
                flags.extend(["-fallow-argument-mismatch", "-fallow-invalid-boz", "-march=armv8.4-a"])
            else:
                flags.extend(["-fallow-argument-mismatch", "-fallow-invalid-boz"])
        if self.spec.satisfies("%gcc@14.1.0") and name == "cflags":
            flags.extend(["-Wno-implicit-function-declaration", "-Wno-implicit-int", "-Wno-incompatible-pointer-types"])
