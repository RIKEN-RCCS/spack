...
class SalmonTddft(CMakePackage):
    ...
    def flag_handler(self, name, flags):
        ...
        if name == "fflags":
            ...
            if self.spec.satisfies("%gcc@14.1.0"):
                flags.append("-fallow-argument-mismatch")
        return (None, None, flags)

    def test_salmon(self):
        test_dir = self.test_suite.current_test_data_dir
        test_file = join_path("C2H2_gs.inp")
        exe_name = self.spec["mpi"].prefix.bin.mpirun
        opts = ' -n 4 '
        opts = opts + (join_path(self.prefix.bin, "salmon"))
        opts = opts + ' < '
        opts = opts + test_file
        opts = opts + ' >& outfile'
        env["OMP_NUM_THREADS"] = "4"
        salmon = which(exe_name)
        os.chdir(test_dir)
        cmms = str(salmon)
        os.system(cmms + opts)
        test_file = join_path(test_dir, "outfile")
        opts = []
        opts.append("check.py")
        scale = which("python3")
        os.chdir(test_dir)
        out = scale(*opts, output=str.split, error=str.split)
        assert "end SALMON" in out
