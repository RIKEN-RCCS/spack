...
class Fds(MakefilePackage):
    ...
    # For GCC compiler
    patch("makefile.patch", when="@6.8.0 target=aarch64:")
    ...
    def test_fds(self):
        ...
        test_file = join_path(test_dir, "simple_test.fds")
        opts = []
        exe_name = self.spec["mpi"].prefix.bin.mpirun
        opts.extend(["-n", "1"])
        opts.append(join_path(self.prefix.bin, "fds"))
        env["OMP_NUM_THREADS"] = "1"
        opts.append(test_file)
        fds = which(exe_name)
        os.chdir(test_dir)
        out = fds(*opts, output=str.split, error=str.split)
        assert "FDS completed successfully" in out
