...
class Frontistr(FrontistrBase):
    ...
    def test_frontistr(self):
        test_dir = self.test_suite.current_test_data_dir
        opts = []
        exe_name = "hecmw_part1"
        env["OMP_NUM_THREADS"] = "2"
        frontistr = which(exe_name)
        os.chdir(test_dir)
        out = frontistr(*opts, output=str.split, error=str.split)
        exe_name = self.spec["mpi"].prefix.bin.mpirun
        opts.extend(["-n", "4"])
        opts.append(join_path(self.prefix.bin, "fistr1"))
        opts.extend(["-t", "2"])
        frontistr = which(exe_name)
        print("##### : ",frontistr)
        print("##### : ",opts)
        out = frontistr(*opts, output=str.split, error=str.split)
        assert "FrontISTR Completed !!" in out
