...
class Openmx(MakefilePackage):
    ...
    def edit(self, spec, prefix):
        ...
        if spec.satisfies("%gcc@14.1.0"):
            makefile.filter("gcc ", "gcc -Wno-implicit-int ".format(prefix))
        mkdirp(prefix.bin)
    ...
    def common_arguments(self):
        ...
        if "%fj" in spec:
            ...
            if "%gcc" in spec:
                ...
                if spec.satisfies("%gcc@14.1.0"):
                    cc_option.append("-Dnosse")
                    cc_option.append("-Wno-implicit-function-declaration")
    ...
    def test_openmx(self):
        test_dir = self.test_suite.current_test_data_dir + "/work"
        test_file = "Fe_Cluster_jx.dat"
        opts = []
        exe_name = self.spec["mpi"].prefix.bin.mpirun
        opts.extend(["-n", "4"])
        opts.append(join_path(self.prefix.bin, "openmx"))
        env["OMP_NUM_THREADS"] = "1"
        opts.append(test_file)
        openmx = which(exe_name)
        os.chdir(test_dir)
        out = openmx(*opts, output=str.split, error=str.split)
        assert "The calculation was normally finished." in out
