...
class Gromacs(CMakePackage, CudaPackage):
    ...
    def test_gromacs(self):
        ...
        if self.spec.satisfies("+mpi"):
            ...
            opts.extend(["-n", "1"])
            opts.append(join_path(self.prefix.bin, "gmx_mpi"))
        else:
            exe_name = "gmx"
        env["OMP_NUM_THREADS"] = "4"
        opts.append("grompp")
        opts.append("-f")
        opts.append("pme_verlet.mdp")
        opts.append("-c")
        opts.append("conf.gro")
        opts.append("-p")
        opts.append("topol.top")
        opts.append("-o")
        opts.append("ions.tpr")
        gromacs = which(exe_name)
        os.chdir(test_dir)
        out = gromacs(*opts, output=str.split, error=str.split)
        ...
        test_file = join_path(test_dir, "md.log")
        opts = []
        opts.append("exe_check.py")
        exch = which("python3")
        os.chdir(test_dir)
        out = exch(*opts, output=str.split, error=str.split)
        assert "GROMACS test Passed" in out
