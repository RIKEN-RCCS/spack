...
class Cp2k(MakefilePackage, CMakePackage, CudaPackage, ROCmPackage):
    ...
    def test_cp2k(self):
        ...
        test_file = join_path(test_dir, "UO2-cell_opt.inp")
        opts = []
        if self.spec.satisfies("+mpi"):
            exe_name = self.spec["mpi"].prefix.bin.mpirun
            opts.extend(["-n", "2"])
            opts.append(join_path(self.prefix.bin, "cp2k.psmp"))
        else:
            exe_name = "cp2k.ssmp"
        ...
        cp2k = which(exe_name)
        os.chdir(test_dir)
        out = cp2k(*opts, output=str.split, error=str.split)
        assert "The number of warnings for this run is : 0" in out
