...
#    """FFX"""
    ...
    def edit(self, spec, prefix):
        if spec.satisfies("@03.01"):
            ...
            m.filter(r'POPT\s=.*$', 'POPT = -P -traditional-cpp -Dcputime')
            m.filter(r'CCOM\s=.*$', 'CCOM = mpifcc')
            m.filter(r'COPT\s=.*$', 'COPT = -Nnoclang -Kvisimpact,ocl -Qt -Dcputime')
            m.filter(r'FCOM\s=.*$', 'FCOM = mpifrt')
            m.filter(r'FOPT\s=.*$', 'FOPT = -Kvisimpact,ocl -Qt -Cpp -Dcputime')
            m.filter(r'LINK\s=.*$', 'LINK = mpiFCC')
            m.filter(r'LOPT\s=.*$', 'LOPT = -Nnoclang --linkfortran -Kvisimpact -Dcputime')
