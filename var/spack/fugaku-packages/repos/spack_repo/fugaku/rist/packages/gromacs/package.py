...
class Gromacs(CMakePackage, CudaPackage):
    ...
    variant(
        "serial", default=True, description="Activate Thread-MPI support (disable for MPI support)"
    )
    conflicts("-serial", when="-mpi")
    ...
    conflicts("+plumed", when="+mpi +serial")
    with when("+plumed"):
        depends_on("plumed+mpi", when="+mpi ~serial")
        depends_on("plumed~mpi", when="~mpi +serial")
    ...
    patch("cleanup-2024.4.patch", when="@2024.4:2024.99")
    ...
    patch("external-kernels-2024.patch", when="@2024:2024.3")
    patch("external-kernels-2024.4.patch", when="@2024.4:2024.99")
    ...
    patch("fftomp-2024.4.patch", when="@2024.4 +fftomp")

    patch("arm_sve-2025.0.patch", when="@2025.0")
    patch("gla2la-2025.0.patch", when="@2025.0")
    patch("sve-2025.patch", when="@2025: +sve")
    patch("external-kernels-2025.patch", when="@2025:")
    patch("pme_simd-2025.patch", when="@2025:")
    patch("pme_spread-2025.patch", when="@2025:")
    patch("tune_pme-2025.patch", when="@2025: ^fujitsu-mpi")
    patch("tune_pme-2025.patch", when="@2025: ^fj-mpi")
    patch("pmswp-2025.patch", when="@2025:")
    patch("std_filesystem_equivalent-2025.patch", when="@2025:")
    patch("essentialdynamics-2025.patch", when="@2025: %fj")
...
class CMakeBuilder(cmake.CMakeBuilder):
    ...
    def cmake_args(self):
        ...
        if target >= "zen4":
            ...
            if self.spec.satisfies("%nvhpc"):
                options.append("-DGMX_SIMD=None")
            elif "sve" in target.features and self.spec.satisfies("+sve"):
        ...
        if self.spec.satisfies("+fast"):
            if "%acfl@24.10" in self.spec:
                options.append("-DCMAKE_C_FLAGS_RELEASE=-O3 -ffp-contract=fast -DNDEBUG")
                options.append("-DCMAKE_CXX_FLAGS_RELEASE=-O3 -ffp-contract=fast -DNDEBUG")
            else:
                options.append("-DCMAKE_C_FLAGS_RELEASE=-Ofast -DNDEBUG")
                options.append("-DCMAKE_CXX_FLAGS_RELEASE=-Ofast -DNDEBUG")

        if self.spec.satisfies("+openmp") and self.spec.variants["openmp_max_threads"].value != "none":
            options.append(
        ...
        if self.spec.satisfies("^[virtuals=lapack] intel-oneapi-mkl"):
            ...
            elif self.spec.satisfies("^acfl") and not self.spec.satisfies("^rist-fftw"):
                options.append(
        ...
        if self.spec.satisfies("+sve"):
            options.append("-DGMX_VERSION_STRING_OF_FORK=fugaku")
        elif self.spec.satisfies("+plumed"):
