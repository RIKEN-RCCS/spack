...
class Cp2k(MakefilePackage, CudaPackage, CMakePackage, ROCmPackage):
    ...
    depends_on("fujitsu-fftw", when="%fj")
    ...
    #    depends_on("spglib@1.16.1~ipo", when="@2023.1:")
    depends_on("spglib", when="+spglib")
    ...
    patch("fj_2025.2.patch", when="@2025.2 %fj")
    ...
    def edit(self, spec, prefix):
        ...
        if "smm=libsmm" in spec:
            ...
            cppflags += ["-D__LIBXSMM"]
#            cppflags += pkgconf("--cflags-only-other", "libxsmmf", output=str).spli
...
#            libs += pkgconf("--libs", "libxsmmf", output=str).split()
        ...
        with open(self.makefile, "w") as mkf:
            ...
            if "%fj" in spec:
                ...
                if "@2025.2" in spec:
                    mkf.write("dbt_tas_mm.o: dbt_tas_mm.F\n")
                    mkf.write("\t$(TOOLSRC)/build_utils/fypp $(FYPPFLAGS) $< $*.F90\n")
                    mkf.write("\t$(FC) -c $(FCFLAGS) -O0 -D__SHORT_FILE__=\"\\\"$(subst $(SRCDIR)/,,$<)\\\"\" -I'$(dir $<)' $(OBJEXTSINCL) $*.F90 $(FCLOGPIPE)\n")
