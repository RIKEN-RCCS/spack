...
from spack.install_test import cache_extra_test_sources
...
class DarshanRuntime(AutotoolsPackage):
    ...
    variant(
        ...
        values=("NONE", "cobalt", "pbs", "sge", "slurm", "fj"),
        multi=False,
    )
    ...
    def configure_args(self):
        ...
        if 'scheduler=slurm' in spec:
            job_id = 'SLURM_JOBID'
        elif 'scheduler=cobalt' in spec:
            job_id = 'COBALT_JOBID'
        elif 'scheduler=pbs' in spec:
            job_id = 'PBS_JOBID'
        elif 'scheduler=sge' in spec:
            job_id = 'JOB_ID'
        elif 'scheduler=fj' in spec:
            job_id = 'PJM_SUBJOBID'
